...
async function updateHabit(h){
   ...
   const result = await fetch(`https://localhost:3000/habits/${h._id}`, options)
...
async function destroy(h) {
   try {
      ...
      await fetch(`https://localhost:3000/habits/${h._id}`, options);
      window.location.reload()
...
async function getUserHabits(username){
   try {
       const response = await fetch(`https://localhost:3000/habits/${username}`);
...
async function requestPostHabit(e){
   ...
   try{
      ...
      const newHabit = await fetch('https://localhost:3000/habits', options);
